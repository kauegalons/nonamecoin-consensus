  ...
  cooperate to register clients and validators, select validators weighted by
  their stake, collect their votes on a transaction, and settle balances when the
  ...
  The system is split into three services, each with its own HTTP port and its
  own SQLite database.
  ...
  proceed with fewer than three of them. Each eligible validator is weighted by
  its stake, then penalized according to how many flags it has accumulated:
  ...
  current directory (seletor/banco/seletor.db), while Management resolves
  site.db relative to its own folder (Management/instance/site.db).
  ...
  - Client passwords are stored in plain text and travel in the URL path, where

    they end up in server logs and browser history.
